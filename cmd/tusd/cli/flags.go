...
func ParseFlags() {
	fs := NewFlagGroupSet(flag.ExitOnError)
	...
	log.SetupStructuredLogger(Flags.VerboseOutput, Flags.LogFormat)
}
	...
	if Flags.EnabledHooksString != "" {
		...
		for i, h := range slc {
			...
			if !slices.Contains(hooks.AvailableHooks, hooks.HookType(h)) {
				log.Stderr.Fatalf("Unknown hook event type in -hooks-enabled-events flag: %s", h)
			}
